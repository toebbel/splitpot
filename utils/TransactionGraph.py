...
def normalizeCycle(original):
    """
    a cycle is normalized, if it starts with the node, that is lexicographically the smalles of all nodes, that occured first in the cycle
    ...
    #find node that has lexicographical the smallest id
    smallestIndex = 0
    for i in range(len(original)):
        if(original[i].userId < original[smallestIndex].userId):
            smallestIndex = i

    #shift the circle, so the smallestIndex is shifted to first position
    return original[smallestIndex:] + original[1:smallestIndex + 1]
