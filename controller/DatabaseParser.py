...
def resolveAlias(alias):
    ...
    with connection:
        cur = connection.cursor()
        cur.execute('SELECT user FROM splitpot_aliases WHERE alias = ?', [alias.lower()])
...
def aliasUserExists(alias, mainMail=None):
    ...
    with connection:
        cur = connection.cursor()
        if mainMail != None:
            cur.execute('SELECT COUNT(*) FROM splitpot_aliases WHERE alias = ? AND user = ?', [alias.lower(), mainMail.lower()])
            exists = cur.fetchone()[0]
        else:
            cur.execute('SELECT COUNT(*) FROM splitpot_aliases WHERE alias = ?', [alias.lower()])
            exists = cur.fetchone()[0]
